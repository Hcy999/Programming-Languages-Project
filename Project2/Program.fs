...
let testBinaryAddition num1 num2 =
    ...
    let resultBinaryList =
        if num1 < 0 && num2 < 0 then
            ...
            printfn "********************"
            if (num1 + num2) < -128 || (num1 + num2) > 127 then
                printfn "Result is out of the 8-bit signed integer range -128 to 127"
            else
...
let testBinarySubtraction num1 num2 =
    ...
    if subtractionResult < -128 || subtractionResult > 127 then
        printfn "Result is out of the 8-bit signed integer range -128 to 127"
    else
        printfn "%d -> %s" subtractionResult (binaryToString finalResult true)
    printfn ""
//
//
//
...
testDecimalToBinary 127
testBinaryToDecimal [0; 0; 1; 1; 0; 0; 0; 0] 
testNegativeDecimalToBinary -128
testBinaryAddition 126 2
testBinarySubtraction 10 11
// Run Logical Operations Test Part
testLogicalOperationsAND 0xF9 0x9F
testLogicalOperationsOR 0x01 0x11
testLogicalOperationsXOR 0xFF 0x88
testLogicalOperationsNOT 0xA5
